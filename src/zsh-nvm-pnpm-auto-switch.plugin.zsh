...
nvm_pnpm_auto_switch_help() {
  ...
  echo "    This command will guide you through configuring your workspace directory,"
  echo "    project listing, and debug mode settings."
  echo ""
  ...
  echo "    Example: nvm_pnpm_auto_switch_workspace ~/projects"
  echo ""
  echo "  nvm_pnpm_auto_switch_list_projects"
  echo "    Toggle automatic project listing when entering your workspace directory."
  echo "    When enabled, the plugin will show a list of available projects in your"
  echo "    workspace directory. When disabled, the plugin will operate silently."
  echo ""
  echo "  nvm_pnpm_auto_switch_debug"
  echo "    Toggle debug mode to show/hide detailed operation information."
  echo "    When enabled, the plugin will output detailed information about what it's"
  echo "    doing, which is useful for troubleshooting."
  echo ""
  echo "  nvm_pnpm_auto_switch_update"
  echo "    Update the plugin to the latest version from GitHub."
  echo "    This command will automatically detect the best update method:"
  echo "    - If the plugin was installed via git clone, it will use 'git pull'"
  echo "    - Otherwise, it will try to use curl or wget to download the latest version"
  echo "    The command will preserve your existing configuration during the update."
  echo ""
  echo "  nvm_pnpm_auto_switch_uninstall"
  echo "    Uninstall the plugin and clean up all related files and settings."
  echo "    This will:"
  echo "    - Remove all environment variables from your .zshenv file"
  echo "    - Remove the plugin from your .zshrc file"
  echo "    - Delete the plugin directory"
  echo "    - Unset all environment variables for the current session"
  echo "    The command will ask for confirmation before proceeding."
  echo ""
  echo "  nvm_pnpm_auto_switch_help"
  echo "    Display this help information."
  echo ""
  echo "  nvm_pnpm_auto_switch_man"
  echo "    Alias for nvm_pnpm_auto_switch_help."
  echo ""
