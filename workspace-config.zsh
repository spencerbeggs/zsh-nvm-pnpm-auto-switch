...
if [[ "$(pwd)" == "$NVM_PNPM_AUTO_SWITCH_WORKSPACE" ]]; then
  ...
  if [[ "$NVM_PNPM_AUTO_SWITCH_LIST_PROJECTS" == "1" ]]; then
    ...
    echo "📂 Welcome to your workspace at $NVM_PNPM_AUTO_SWITCH_WORKSPACE!"
    ...
    echo "\nTo disable project listing, run: nvm_pnpm_auto_switch_list_projects"
  elif [[ "$NVM_PNPM_AUTO_SWITCH_DEBUG" == "1" ]]; then
    # Only show messages in debug mode if project listing is off
    echo "[zsh-nvm-pnpm-auto-switch] In workspace directory (project listing disabled)"
    echo "[zsh-nvm-pnpm-auto-switch] To enable project listing, run: nvm_pnpm_auto_switch_list_projects"
  fi
